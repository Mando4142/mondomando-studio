...
app.post('/api/webhook', express.raw({type: 'application/json'}), (req, res) => {
    ...
    if (event.type === 'checkout.session.completed') {
        ...
        if (session.metadata && session.metadata.action === 'priority_boost') {
            const artistLower = session.metadata.artistLower;
...
app.post('/api/checkout/priority', async (req, res) => {
    const session = await stripe.checkout.sessions.create({
        ...
        metadata: { action: 'priority_boost', artistLower: req.body.artist.toLowerCase() }
    });
    res.json({ url: session.url });
});

app.get('/api/stripe/history', checkAdminAuth, (req, res) => {
    // Hier könntest du die Stripe-Session-Historie abfragen
    res.json([]); 
});
